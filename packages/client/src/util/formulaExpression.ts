...
function EvaluateLessThan(values: number[]): number {
  // [1, 2, 3, 4] -> true(1)
  return values
    .map((_, i, array) => (i ? array[i] - array[i - 1] : null))
...
function EvaluateMatch(
  referencedStats: ReferenceVariables,
  caseOperators: CaseOperatorType[],
): number {
  const matchedCase = caseOperators.find(
    (caseOperator) =>
      caseOperator.operator === 'default' ||
      EvaluateOperand(referencedStats, caseOperator.operands[0]),
  );
...
export function evaluateFormula(
  ...
  referencedStats: ReferenceVariables = defaultReferencedStats,
) {
