...
NHK学生ロボコンのようなロボットコンテストの大会で、対戦チームの得点を入力し、それを会場のスクリーンやライブストリーミングに表示する統合的なシステムです。
...
サーバ側でクライアント側のビルド済みファイルをホスティングしているので、実際の大会時には次のような運用を想定しています。
...
config.jsonを読み書きしやすいyamlに変更したり、TypeScriptの型を使っていい感じにバリデーションしたり、サーバとクライアントで2つのファイルを用意しなきゃいけないのを改善したりしたい。

#### 各試合のログ保存・リプレイ機能

各試合でチームの点数状況などを保存したり、得点の増減など各操作を記録して再現したりする機能をつけたい。
